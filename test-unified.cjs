const { unifiedAgentFlow, debugUnifiedFlow } = require('./dist/mcp/unified-agent-flow.cjs');

// Mock das ferramentas para testes
const mockTools = {
  enhancedCatalogTool: {
    name: 'enhanced_catalog_search',
    description: 'Busca veículos no catálogo Honda com verificação de stock opcional',
    handler: async ({ query, checkStock }) => {
      console.log(`🛒 Catalog Tool: "${query}" | Stock Check: ${checkStock}`);
      
      if (checkStock) {
        return `**[FONTE: CATÁLOGO]** PCX 125: €3.590 (5 unidades) | Forza 350: €6.290 (3 unidades) | BigQuery: YES`;
      } else {
        return `**[FONTE: CATÁLOGO]** PCX 125: €3.590 | Forza 350: €6.290 | SH 125: €3.190 | BigQuery: NO (token saving)`;
      }
    }
  },
  ...
  enhancedManualsTool: {
    ...
    handler: async ({ query }) => {
      console.log(`📖 Manuals Tool: "${query}"`);
      return `**[FONTE: MANUAL]** PCX 125: Pressão dianteira 29 psi (2.0 bar), traseira 33 psi (2.3 bar)`;
    }
  },
  ...
  analyticsTool: {
    ...
    handler: async ({ metric }) => {
      console.log(`📊 Analytics Tool: "${metric}"`);
      return `**[FONTE: ANALYTICS]** 15 utilizadores ativos agora`;
    }
  }
};

// Mock do unified agent flow
const mockUnifiedAgentFlow = async (userInput) => {
  ...
  if (manualKeywords.some(keyword => lowerQuery.includes(keyword))) {
    toolUsed = "manuals";
    reply = await mockTools.enhancedManualsTool.handler({ query: userInput });
  } else if (catalogKeywords.some(keyword => lowerQuery.includes(keyword))) {
    toolUsed = "catalog";
    bigQueryAccess = needsStock;
    reply = await mockTools.enhancedCatalogTool.handler({ query: userInput, checkStock: needsStock });
  } else if (analyticsKeywords.some(keyword => lowerQuery.includes(keyword))) {
    toolUsed = "analytics";
    reply = await mockTools.analyticsTool.handler({ metric: "active_users" });
  }
...
async function runUnifiedTests() {
  ...
  try {
    const debugResult = await debugUnifiedFlow({ 
      query: "Teste debug", 
      forceTool: "catalog",
      forceStockCheck: true 
    });
    
    console.log(`✅ Debug mode: Tool=${debugResult.debug?.toolDecision?.tool} | Stock=${debugResult.debug?.bigQueryAccess}`);
  } catch (error) {
